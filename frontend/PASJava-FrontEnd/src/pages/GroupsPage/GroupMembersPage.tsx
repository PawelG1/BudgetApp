...
const GroupMembersPage = ({ group, onBack }: Props) => {
  ...
  const applyMembersResults = useCallback(
    ([membersResult, debtsResult]: Awaited<ReturnType<typeof loadMembersData>>) => {
      ...
      if (membersResult.status === "fulfilled") {
        ...
        console.error("Bˆ¥d pobierania czˆonk¢w grupy:", membersResult.reason);
        setMembers([]);
        setErrorMessage((current) =>
          current || "Nie udaˆo si© pobra† czˆonk¢w grupy."
        );
      }
      ...
      if (debtsResult.status === "fulfilled") {
        ...
        console.error("Bˆ¥d pobierania dˆug¢w grupy:", debtsResult.reason);
        setDebts([]);
        setErrorMessage((current) =>
          current || "Nie udaˆo si© pobra† dˆug¢w grupy."
        );
      }
    },
    []
  ...
  return (
    <div className={styles.container}>
      ...
      <AddGroupTransaction groupId={group.id} onTransactionAdded={refreshMembers} />
